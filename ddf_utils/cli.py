...
@click.option(
    ...
    flag_value=True,
    default=False,
    help="do not use default excludes",
)
def cleanup(path, how, force, exclude, no_default_exclude):
    ...
    if force:
        ...
        if not is_datapackage(path):
            print(
                "not a dataset path: {}. Please set correct path or "
...
def create_datapackage(path, update, overwrite, progress_bar):
    ...
    if not update and not overwrite:
        if os.path.exists(os.path.join(path, "datapackage.json")):
            click.echo(
                "datapackage.json already exists. use --update to update or --overwrite to create new"
            )
            return
        ...
        if overwrite:
            ...
            res = get_datapackage(
                path, use_existing=True, update=True, progress_bar=progress_bar
            )
...
@click.option(
    "--update", "update", flag_value=False, help="Don't use. Not implemented yet"
)
@click.option(
    "--dry_run",
    "-d",
...
def run_recipe(recipe, outdir, ddf_dir, update, dry_run, gen_dp, show_tree):
    ...
    if serve and gen_dp:
        ...
        if os.path.exists(datapackage_path):
            ...
            shutil.copyfile(
                datapackage_path, os.path.join(outdir, "datapackage.json.bak")
            )
...
@click.option(
    ...
    help="split file type",
    default="csv",
)
@click.option(
    "--overwrite/--no-overwrite", default=False, help="overwrite existing files or not"
)
...
@click.option(
    "--type", "-t", "dtype", type=click.Choice(["json", "csv"]), help="split file type"
)
@click.option("--split_path", default="langsplit", help="path to langsplit folder")
...
def diff(dataset1, dataset2, git, checkout_path, indicator, on_key, git_base_path):
    ...
    if git:
        ...
        assert is_datapackage(git_base_path), (
            "please run this command in a dataset dir."
        )
        ...
        try:
            ...
            logging.info(
                "checkout git rev {} into {}".format(dataset1, join(checkout_path, p1))
            )
            os.system("git -C {} checkout {}".format(git_base_path, p1))
            os.system(
                "git -C {} checkout-index -a -f --prefix={}/".format(
                    git_base_path, join(checkout_path, p1)
                )
            )
        ...
        try:
            ...
            logging.info(
                "checkout git rev {} into {}".format(dataset2, join(checkout_path, p2))
            )
            os.system("git -C {} checkout {}".format(git_base_path, p2))
            os.system(
                "git -C {} checkout-index -a -f --prefix={}/".format(
                    git_base_path, join(checkout_path, p2)
                )
            )
        except FileExistsError:
            pass
...
def from_csv(input, out_path):
    ...
    if os.path.isfile(input):
        ...
        files = [
            os.path.join(input, x) for x in os.listdir(input) if x.endswith(".csv")
        ]
...
def etl_type(script_dir):
    ...
    if not os.path.exists(script_dir):
        logging.info(
            f"Script directory {script_dir} does not exist, assuming manual ETL."
        )
        click.echo("manual,")
        return

    # Clear any cached etl module to ensure fresh import
    if "etl" in sys.modules:
        del sys.modules["etl"]

    sys.path.insert(0, script_dir)
    try:
        import etl  # type: ignore[import-not-found]
        ...
        # Clean up: remove the script_dir from sys.path and clear the etl module
